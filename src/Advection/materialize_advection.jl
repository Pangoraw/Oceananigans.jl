...
materialize_advection(u::OnlySelfUpwinding, grid) = OnlySelfUpwinding(materialize_advection(u.cross_scheme, grid),
                                                                      u.δU_stencil, u.δV_stencil, u.δu²_stencil, u.δv²_stencil)

materialize_advection(u::CrossAndSelfUpwinding, grid) = CrossAndSelfUpwinding(materialize_advection(u.cross_scheme, grid),
...
materialize_advection(weno::WENO{N, FT, WCT}, grid) where {N, FT, WCT} =
    WENO{N, FT, WCT}(weno.bounds,
                     ...
                     materialize_advection(weno.time_discretization, grid))
...
materialize_advection(scheme::UpwindBiased{N, FT}, grid) where {N, FT} =
    UpwindBiased{N, FT}(materialize_advection(scheme.buffer_scheme, grid),
                        ...
                        materialize_advection(scheme.time_discretization, grid))

materialize_advection(scheme::Centered{N,FT}, grid) where {N,FT} =
    Centered{N, FT}(materialize_advection(scheme.buffer_scheme, grid), materialize_advection(scheme.time_discretization, grid))
